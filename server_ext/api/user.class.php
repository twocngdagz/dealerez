...
//FUNCTION WILL RETURN THE LIST OF USER BASED ON THEIR REGION
...
//FUNCTION WILL RETURN THE LIST OF USER BASED ON THEIR LISTING TYPE( vans, coupe, sedan, etc. .)
function get_dealers_type($is_active=false,$type=false,$orderby = false,$sortby = "DESC",$slimit=false,$elimit=false)
{
			...
			if($type) {
				$type = "and body_style like '".$type."%' GROUP BY users.user_id";
			}
			if($type=="") {
				$type = "GROUP BY users.user_id";
			}
			if($elimit){
				$elimit = "LIMIT $slimit, $elimit";
			}			
			if($is_active) //for active users only
			{
				$sql="SELECT * FROM listing INNER JOIN users ON listing.user_id = users.user_id WHERE users.is_active=1 $type $orderby $elimit ";
			}
			else //for inactive users only
			{
				$sql="SELECT * FROM listing INNER JOIN users ON listing.user_id = users.user_id WHERE users.is_active=0 $type $orderby $elimit";
			}
			$result=Execute_command($sql);
			//echo $sql;
			try
...
//FUNCTION WILL RETURN THE LIST OF USER BASED ON THEIR LISTING FEATURED( vans, coupe, sedan, etc. .)
function get_is_featured_dealers($is_active=false,$is_featured=false,$orderby = false,$sortby = "DESC",$slimit=false,$elimit=false)
{
			$user_info=array();
			...
			if($is_featured) {
				$is_featured = "and listing.is_featured=1 GROUP BY users.user_id";
			}
			if($elimit){
				$elimit = "LIMIT $slimit, $elimit";
			}			
			if($is_active) //for active users only
			{
				$sql="SELECT * FROM listing INNER JOIN users ON listing.user_id = users.user_id WHERE users.is_active=1 $is_featured $orderby $elimit ";
			}
			else //for inactive users only
			{
				$sql="SELECT * FROM listing INNER JOIN users ON listing.user_id = users.user_id WHERE users.is_active=0 $is_featured $orderby $elimit";
			}
			$result=Execute_command($sql);
			//echo $sql;
			try
...
//FUNCTION WILL RETURN THE LIST OF USER BASED ON THEIR LISTING FEATURED( vans, coupe, sedan, etc. .)
function get_is_dicker_dealers($is_active=false,$is_dicker=false,$orderby = false,$sortby = "DESC",$slimit=false,$elimit=false)
{
			$user_info=array();
			...
			if($is_dicker) {
				$is_dicker = "and listing.is_dicker=1 GROUP BY users.user_id";
			}
			if($elimit){
				$elimit = "LIMIT $slimit, $elimit";
			}			
			if($is_active) //for active users only
			{
				$sql="SELECT * FROM listing INNER JOIN users ON listing.user_id = users.user_id WHERE users.is_active=1 $is_dicker $orderby $elimit ";
			}
			else //for inactive users only
			{
				$sql="SELECT * FROM listing INNER JOIN users ON listing.user_id = users.user_id WHERE users.is_active=0 $is_dicker $orderby $elimit";
			}
			$result=Execute_command($sql);
			//echo $sql;
			try
